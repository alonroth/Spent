...
export function getAnnualTable(workspaceId: number, year: number): AnnualTablePayload {
  ...
  const years=(db.prepare(`SELECT DISTINCT CAST(substr(date,1,4) AS INTEGER) AS year FROM transactions WHERE workspace_id=? UNION SELECT ? ORDER BY year DESC`).all(workspaceId,new Date().getFullYear()) as {year:number}[]).map(r=>r.year);
